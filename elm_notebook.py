...
def custom():
    ...
    for ex in data:
        target.append(ex[37])
    TARGET =  numpy.array(target) #target[:1000]
    #data = data[:37,]
    #data.shape
    DATA = []
    ...
    return numpy.array(DATA).astype(numpy.float),TARGET.astype(numpy.float) #data also :1000
...
""" MAIN DATASET  """
cusx, cusy = custom()
print len(cusx), len(cusy)
cusx_train, cusx_test, cusy_train, cusy_test = train_test_split(cusx, cusy, test_size=0.2)
...
elmc = ELMClassifier(n_hidden=1000, activation_func='multiquadric', alpha=0.001, random_state=0)
